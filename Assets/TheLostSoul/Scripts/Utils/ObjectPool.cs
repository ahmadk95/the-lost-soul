...
namespace tls.utils
{
    public sealed class ObjectPool : MonoBehaviour
    {
        ...
        void OnDisable()
        {
            foreach (var key in spawnedObjects.Keys)
            {
                Destroy(key);
            }
        ...
        public IEnumerator WaitAndRecycle(float time, System.Action OnExpire)
        {
            yield return new WaitForSeconds(time);
            yield return null;
            //MainThreadDispatcher.Instance.Add(OnExpire);
        }
        ...
        public static void Recycle(GameObject obj, float time)
        {
            GameObject prefab;
            if (instance.spawnedObjects.TryGetValue(obj, out prefab))
                instance.StartCoroutine(instance.WaitAndRecycle(time, () => Recycle(obj, prefab)));
            else
                GameObject.Destroy(obj);
        }
    ...
    public static class ObjectPoolExtensions
    {
        ...
        public static void Recycle<T>(this T obj, float time) where T : Component
        {
            ObjectPool.Recycle(obj, time);
        }
        public static void Recycle(this GameObject obj, float time)
        {
            ObjectPool.Recycle(obj, time);
        }
